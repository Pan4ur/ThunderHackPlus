...
vec3 getColor(vec4 centerCol, vec2 uv2) {
    ...
    vec4 outputI = forCol2 + vec4(backCol2, 0.0);
    return vec3(outputI[0], outputI[1], outputI[2]);
...
void main() {
    ...
    if(centerCol.a != 0) {
        ...
        for (float x = -radius; x < radius; x++) {
            for (float y = -radius; y < radius; y++) {
                ...
                if (alpha0 == -1.0) {
                    ...
                    uv2 = 2. * gl_FragCoord.xy / resolution.xy - 1.;
                    gl_FragColor = vec4(getColor(centerCol, uv2), alpha0);
                    return;
                }
            }
        }
        if (alphaOutline > 0) {
            uv2 = 2. * gl_FragCoord.xy / resolution.xy - 1.;
            colorFinal = getColor(centerCol, uv2);
        }
