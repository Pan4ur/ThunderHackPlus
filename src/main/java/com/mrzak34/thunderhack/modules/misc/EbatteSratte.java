...
public class EbatteSratte extends Module {
    ...
    String[] Erp = new String[]{
        ...
        String[] Funny = new String[]{
            ...
            "Казахстан угражает бамбадировки!",
            "Чел ты гений",
            "Ты понимаешь что я чепаев",
            "Бархатные тяги уфффф кэфтэмэ",
            "Чел ты просто нулёвый",
            "Стартуем!",
            "Пон",
            "Яблоки Не Жруться Что Делать!!!",
            "ЕБАТЬ ТЫ ВЕРТОЛЕТ Я ТУПО В УНИТАЗЕ ТЫ НЕ ПОНЯЛ ПОМАДА СЛОНА?",
            "Абоба",
            "А ВОТ ТЫ ЗНАЛ ЧТО МОЛОДОЙ ОГУРЕЦ СПОСОБСТВУЕТ РАСТВОРЕНИЮ ВСЕЛЕННОЙ НА КРИТИЧЕСКОМ УРОВНЕ ГЕНЕТИКИ ВСЕМИРНЫХ ОБЕЗЬЯН КОТОРЫЕ ЛЮБЯТ ТРОГАТЬ ЛУНУ ХВОСТАМИ С БУТЫЛКИ ИЗ ПОД ШАМПАНСКОГО",
            "ВСЕМЫРНЫЕ НОВОСТИ СООБЩАЮТ ЧТО ДИНАСТИЯ ПОМИДОР СОБИРАЕТСЯ СРАЗИТЬСЯ С КОРОЛЕВЧТВОМ ПИЛОЧЕК ДЛЯ НОГТЕЙ!! ОПАСНОСТЬ!!! ОПАСНОСТЬ!!! В ОЧКО ПРИРОДЫ ВХОДЯТ 2 АМБАЛА!!!",
