...
public class EffectsRemover extends Module{
    public EffectsRemover() {
        super("PowderTweaks", "Убирает джампбуст от-пороха и юзает его автоматом", Module.Category.FUNNYGAME, true, false, false);
    }
        ...
        if (this.jumpBoost.getValue() ) {
            if (Thunderhack.moduleManager.getModuleByClass(Aura.class).isEnabled()){
                if (mc.player.isPotionActive(MobEffects.JUMP_BOOST)) {
                    ...
                    EffectsRemover.mc.player.removeActivePotionEffect(Potion.getPotionFromResourceLocation((String)"jump_boost"));
                    jboost = true;
                }
            }
        }
