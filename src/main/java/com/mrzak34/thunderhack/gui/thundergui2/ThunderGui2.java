...
public class ThunderGui2 extends GuiScreen {
    ...
    public void renderGui(int mouseX, int mouseY, float partialTicks){
        ...
        // Основная плита
        RoundedShader.drawRound(main_posX, main_posY, main_width, main_height, 9f, new Color(37, 27, 41, 250));


        // Плита с лого
        RoundedShader.drawRound(main_posX + 5, main_posY + 5, 90, 30, 7f, new Color(50, 35, 60, 250));
        FontRender.drawString2("THUNDERHACK+", main_posX + 10, main_posY + 15, -1);
        FontRender.drawString5("v2.40", main_posX + 75, main_posY + 30, new Color(0x656565).getRGB());

        // Левая плита под категриями
        RoundedShader.drawRound(main_posX + 5, main_posY + 40, 90, 140, 7f, new Color(50, 35, 60, 178));

        // Выбор между CfgManager и FriendManager
        if (currentMode == CurrentMode.Modules) {
            RoundedShader.drawRound(main_posX + 20, main_posY + 195, 60, 20, 4f, new Color(50, 35, 60, 178));
        } else if (currentMode == CurrentMode.ConfigManager) {
            RoundedShader.drawGradientRound(main_posX + 20, main_posY + 195, 60, 20, 4f, new Color(133, 93, 162, 178), new Color(133, 93, 162, 178), new Color(50, 35, 60, 178), new Color(50, 35, 60, 178));
        } else {
            RoundedShader.drawGradientRound(main_posX + 20, main_posY + 195, 60, 20, 4f, new Color(50, 35, 60, 178), new Color(50, 35, 60, 178), new Color(133, 93, 162, 178), new Color(133, 93, 162, 178));
        }


        RoundedShader.drawRound(main_posX + 49.5f, main_posY + 197, 1, 16, 0.5f, new Color(88, 64, 107, 178));

        FontRender.drawMidIcon("u", main_posX + 20, main_posY + 195, currentMode == CurrentMode.ConfigManager ? -1 : new Color(0x8D8D8D).getRGB());
        FontRender.drawMidIcon("v", main_posX + 54, main_posY + 196, currentMode == CurrentMode.FriendManager ? -1 : new Color(0x8D8D8D).getRGB());
        ...
        if (first_open) {
            ...
            RoundedShader.drawRound((float) (main_posX + 8), (float) CategoryY + slider_y, 84, 15, 2f, new Color(25, 20, 30, 255));
            first_open = false;
        } else {
            if (currentMode == CurrentMode.Modules)
                RoundedShader.drawRound((float) (main_posX + 8), (float) (RenderUtil.interpolate(CategoryY, prevCategoryY, category_animation)) + slider_y, 84, 15, 2f, new Color(25, 20, 30, 255));
        }
        ...
        if (currentMode != CurrentMode.Modules) {
            ...
            RoundedShader.drawRound(main_posX + 100, (float) main_posY + 40, (float) 295, (float) main_height - 44, 4f, new Color(25, 20, 30, 255));
            ...
            RenderUtil.draw2DGradientRect(main_posX + 102, main_posY + 34, main_posX + 393, main_posY + 60, new Color(25, 20, 30, 0).getRGB(), new Color(25, 20, 30, 255).getRGB(), new Color(25, 20, 30, 0).getRGB(), new Color(37, 27, 41, 245).getRGB());
            RenderUtil.draw2DGradientRect(main_posX + 102, main_posY + main_height - 35, main_posX + 393, main_posY + main_height, new Color(25, 20, 30, 255).getRGB(), new Color(25, 20, 30, 0).getRGB(), new Color(25, 20, 30, 255).getRGB(), new Color(37, 27, 41, 0).getRGB());
        ...
        if (currentMode == CurrentMode.ConfigManager) {
            ...
            FontRender.drawCentString6("+", main_posX + 378, main_posY + 19, -1);
        }
        if (currentMode == CurrentMode.FriendManager) {
        ...
        if (selected_plate != null && settings_animation < 0.9999) {
            RoundedShader.drawRound((float) RenderUtil.interpolate(main_posX + 200, selected_plate.getPosX(), settings_animation), (float) RenderUtil.interpolate(main_posY + 40, selected_plate.getPosY(), settings_animation), (float) RenderUtil.interpolate(195, 90, settings_animation), (float) RenderUtil.interpolate(main_height - 45, 30, settings_animation), 4f, ColorUtil.applyOpacity(new Color(25, 20, 30, 255), 1f - settings_animation));
        }
        GL11.glDisable(GL11.GL_SCISSOR_TEST);
    }
