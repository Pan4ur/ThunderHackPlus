...
public class TargetHud extends HudElement {
    ...
    public void renderTHud(Render2DEvent e) {
        ...
        sizeAnimation(getPosX() + 75 + animX.getValue(), getPosY() + 25 + animY.getValue(), animation.getAnimationd());
        ...
        if (animation.getAnimationd() > 0) {
            ...
            // Основа
            Particles.roundedRect(getPosX(), getPosY(), 70, 50, 12, new Color(0, 0, 0, 139));
            Particles.roundedRect(getPosX() + 50, getPosY(), 100, 50, 12, new Color(0, 0, 0, 255));
            //
            ...
            if (imageMode.getValue() != ImageModeEn.None) {
                ...
                if (imageMode.getValue() == ImageModeEn.Anime)
                    renderTexture(getPosX() + 50, getPosY(), 0, 0, 100, 50, 100, 50, 100, 50);
                else
                    renderCustomTexture(getPosX() + 50, getPosY(), 0, 0, 100, 50, 100, 50, 100, 50);
                Stencil.dispose();
                GL11.glPopMatrix();
            ...
            GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
            fastRoundedRect(getPosX() + 5.5f + hurtPercent2, getPosY() + 5.5f + hurtPercent2, getPosX() + 44 - hurtPercent2 * 2, getPosY() + 44 - hurtPercent2 * 2, 6F);
            ...
            renderPlayerModelTexture(getPosX() + 5.5f + hurtPercent2, getPosY() + 5.5f + hurtPercent2, 3F, 3F, 3, 3, (int) (39 - (double) hurtPercent2 * 2), (int) (39 - (double) hurtPercent2 * 2), 24, 24.5f, (AbstractClientPlayer) target);
            renderPlayerModelTexture(getPosX() + 5.5f + hurtPercent2, getPosY() + 5.5f + hurtPercent2, 15F, 3F, 3, 3, (int) (39 - (double) hurtPercent2 * 2), (int) (39 - (double) hurtPercent2 * 2), 24, 24.5f, (AbstractClientPlayer) target);
            ...
            RenderUtil.drawBlurredShadow(getPosX() + 54, getPosY() + 34 - 12, 90, 8, blurRadius.getValue(), a);

            RoundedShader.drawGradientRound(getPosX() + 55, getPosY() + 35 - 12, 90, 8, 2f, a.darker().darker(), b.darker().darker().darker().darker(), c.darker().darker().darker().darker(), d.darker().darker().darker().darker());
            RoundedShader.drawGradientRound(getPosX() + 55, getPosY() + 35 - 12, 90 * (health / 20), 8, 2f, a, b, c, d);
            if (hpMode.getValue() == HPmodeEn.HP) {
                FontRender.drawCentString6(String.valueOf(Math.round(10.0 * health) / 10.0), getPosX() + 100, getPosY() + 25.5f, -1);
            } else {
                FontRender.drawCentString6(((Math.round(10.0 * health) / 10.0) / 20f) * 100 + "%", getPosX() + 100, getPosY() + 25.5f, -1);
            }
            //
            ...
            for (ItemStack itemStack : items) {
                ...
                GL11.glPushMatrix();
                GL11.glTranslated(xItemOffset, getPosY() + 35, 0);
            ...
            if (!Thunderhack.moduleManager.getModuleByClass(NameProtect.class).isEnabled()) {
                FontRender.drawString6(target.getName() + " | " + (double) Math.round(10.0 * mc.player.getDistance(target)) / 10.0 + " m", getPosX() + 55, getPosY() + 5, -1, false);
            } else {
                FontRender.drawString6("Protected | " + (double) Math.round(10.0 * mc.player.getDistance(target)) / 10.0 + " m", getPosX() + 55, getPosY() + 5, -1, false);
            }
            //Поушены
    ...
    private void drawPotionEffect(EntityPlayer entity) {
        ...
        FontRender.drawString7(finalString.toString(), getPosX() + 55, getPosY() + 14, new Color(0x8D8D8D).getRGB(), false);
    }
    ...
    public boolean isHovering() {
        return normaliseX() > getPosX() && normaliseX() < getPosX() + 150 && normaliseY() > getPosY() && normaliseY() < getPosY() + 50;
    }
